...
# Reformat code style
format:
	$(poetry_exec) run black dnev2spb && \
	git add -u && \
	$(poetry_exec) run isort dnev2spb && \
	git add -u && \
	$(poetry_exec) run autoflake \
		--ignore-init-module-imports \
		--remove-unused-variables \
		--recursive \
		--in-place dnev2spb tests && \
	git add -u

# Run tests locally
test:
	$(poetry_exec) run pytest tests --cov=dnev2spb --cov-report=html

# Tests command for CI with .coveragerc report
test-ci:
	$(poetry_exec) run coverage run --source=dnev2spb -m pytest tests
